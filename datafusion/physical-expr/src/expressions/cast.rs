...
mod tests {
    ...
    #[test]
    fn test_cast_utf8_to_time64() -> Result<()> {
        let original = vec!["08:09:10.123456789"];
        use arrow::array::ArrayRef;
        use arrow::compute;
        let a1 = Arc::new(StringArray::from(original.clone())) as ArrayRef;
        let expected: Vec<Option<i64>> =
            compute::cast(&a1, &DataType::Time64(TimeUnit::Nanosecond))
                .unwrap()
                .as_any()
